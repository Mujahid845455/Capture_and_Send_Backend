...
# capture_and_send.py
# Production-ready Socket.IO server for Sign Language Prediction
...
# Try to import critical server libraries
try:
    import socketio
    import eventlet
    ...
    print("Please run: pip install python-socketio eventlet flask")
    sys.exit(1)
...
try:
    ...
    print("⚠️ ML libraries (tensorflow/sklearn) missing. Running in dummy mode.")
    tf = None
...
# Basic routing middleware to support health checks
def middleware(environ, start_response):
    if environ['PATH_INFO'] == '/health':
        return health_check(environ, start_response)
    # Default to Socket.IO for all other paths
...
@sio.on("process_landmarks")
def handle_landmarks(sid, data):
    global predicted_sentence, last_predicted_label, last_prediction_time
    
    try:
        right_hand = data.get("right_hand")
        left_hand = data.get("left_hand")
        
        # Priority on right hand if both present (matching training bias)
        hand_lms = right_hand or left_hand
        handedness = "Right" if right_hand else "Left"

        if not hand_lms or len(hand_lms) < 21:
            return

        # Feature Extraction & Normalization (Wrist-relative)
        wrist = hand_lms["0"]
        features = []
        for i in range(21):
            lm = hand_lms[str(i)]
            ...
            # Flip X for right hand to match left-hand training set
            if handedness == "Right":
                x = -x
        ...
        if len(stabilization_window) == stabilization_window.maxlen:
            ...
            if counts[most_common] >= stabilization_threshold and confidence > 0.4:
                stable_letter = most_common

        # Sentence Formation
        current_time = time.time()
        if stable_letter and stable_letter != "nothing":
            if stable_letter != last_predicted_label or (current_time - last_prediction_time > cooldown_time):
                if stable_letter == "space":
                    predicted_sentence += " "
                elif stable_letter == "del":
                    predicted_sentence = predicted_sentence[:-1]
                else:
                    predicted_sentence += stable_letter
                
                print(f"✨ Predicted: {stable_letter} | Sentence: {predicted_sentence}")
                last_predicted_label = stable_letter
                last_prediction_time = current_time
        ...
        response = {
            ...
            "sign_language": {
                "letter": stable_letter or letter,
                "confidence": confidence,
                "sentence": predicted_sentence
            },
            "timestamp": data.get("timestamp")
        }
        sio.emit("landmarks_processed", response, to=sid)
...
if __name__ == "__main__":
    ...
    try:
        # Using middleware allows both Socket.IO and the '/health' route to coexist
        eventlet.wsgi.server(eventlet.listen(('0.0.0.0', PORT)), middleware)
    except Exception as e:
